...
get_number_of_adv_pages <- function(marktplaats_url) {
  marktplaats_iphone <- read_html(marktplaats_url) %>% 
    html_node("a:nth-child(13)") %>%
    html_text() %>%
    as.numeric()
}
...
get_adv_urls_from_page <- function(page_html) {
  page_html %>% 
    html_nodes(".listing-title-description") %>% 
    html_nodes("h2") %>% 
    html_nodes("a") %>% 
    html_attr("href") %>% 
    return()
...
combine_adv_info_from_page <- function(titles, urls, filter_admarkt = TRUE) {
  if(length(titles) == length(urls) & length(titles) > 0 ) {
    adv_info <- data.frame(
      ...
      mutate(
        url = str_replace(url,"html?.*","html"),
        adv_id = get_adv_id(url)
      )
    # Filter admarkt advs if required
    if(filter_admarkt) {
      adv_info %>% 
        filter(!grepl("admarkt.marktplaats.nl",url)) %>% 
        return()
    } else {
      return(adv_info)
    }
